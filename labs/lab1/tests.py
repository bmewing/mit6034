...
def family_rules_1_testanswer(val, original_val=None):
    return set(
        [
            x
            for x in val
            if x.split()[0] in ("father", "son", "daughter", "brother", "sister",)
        ]
...
result_bc_2 = OR(
    ...
    AND(
        OR(
            "alice is a bird",
            "alice has feathers",
            AND("alice flies", "alice lays eggs"),
        ),
...
result_bc_3 = OR(
    ...
    AND(
        OR(
            "geoff is an ungulate",
            AND(
                OR("geoff is a mammal", "geoff has hair", "geoff gives milk"),
                "geoff has hoofs",
            ),
            AND(
                OR("geoff is a mammal", "geoff has hair", "geoff gives milk"),
